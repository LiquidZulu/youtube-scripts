...
export default makeScene2D(function* (view) {
  ...
  function* showRay(i: number) {
    yield* all(
      reasons.ray[i].end(1, 1),
      delay(0.2, reasons.txt[i].opacity(1, 1)),
    );
  }
  yield* showRay(0);

  yield* waitUntil("reason 2");

  yield* showRay(1);

  yield* waitUntil("reason 3");

  yield* showRay(2);

  yield* waitUntil("conflict displayed");
  ...
  yield* waitUntil("one action excludes the other");
  yield* all(
    rays[0].opacity(1, 1),
    vids[0].opacity(1, 1),
  ...
  yield* waitUntil("they are mutually-exclusive");
  yield* all(
    rays[1].opacity(1, 1),
    vids[1].opacity(1, 1),
    rays[0].opacity(0.2, 1),

...
export default makeScene2D(function* (view) {
  ...
  yield* waitUntil("description in");
  ...
  yield* waitUntil("real world not real");
  yield* list().next();
  yield* waitUntil("real world colourless etc");
  yield* list().next();
  yield* waitUntil("all that exists is quantity and motion");
  yield* list().next();
  ...
  yield* waitUntil("thought invalidates");
  yield* rays[0].end(1, 1);
  yield* waitUntil("that which it is based upon");
